#!/usr/bin/env python3
"""
Comprehensive test of iteration 3 controller with proper distance handling
"""
...
from progressive_deceleration_iteration_3 import (
    ProgressiveDecelerationV3
)
...
def simulate_scenario(controller, scenario: EmergencyScenario,
    ...
    # Run simulation
    while v_ego > effective_target and distance_traveled < scenario.distance_to_curve_m * 1.5:
        ...
        result = controller.update(
            ...
            actual_distance_to_curve=remaining_distance  # Key fix!
        )
        ...
        if v_ego <= effective_target * 1.05:
            break

    # Results
    success = v_ego <= effective_target * 1.1
    intervention_avoided = max_emergency_level != EmergencyLevel.INTERVENTION
...
def test_all_scenarios():
    ...
    print("Testing Iteration 3 Controller - With Proper Distance Handling")
    print("="*80)
    print("\nSuccess Criteria:")
    ...
    for scenario in EMERGENCY_SCENARIOS:
        controller = ProgressiveDecelerationV3()  # Fresh controller
    ...
    if criteria_not_met:
        ...
        if failures:
            ...
            for name, result in failures[:5]:  # Show first 5
                print(f"  {name}: needed {result.max_decel_g:.2f}g, "
                      f"reached {result.emergency_level_reached.name}")

        intervention_scenarios = [(name, r) for name, r in results.items()
                                 if not r.intervention_avoided]
        if intervention_scenarios:
            print(f"\nIntervention required ({len(intervention_scenarios)}):")
            for name, _ in intervention_scenarios[:3]:
                print(f"  {name}")

    return results, len(criteria_met) >= 4  # Success if 4/5 criteria met
...
if __name__ == "__main__":
    ...
    if meets_criteria:
        print("\n" + "="*80)
        print("SUCCESS! Iteration 3 meets the criteria.")
        print("="*80)
    else:
        print("\n" + "="*80)
        print("Iteration 3 still needs refinement.")
        print("="*80)
