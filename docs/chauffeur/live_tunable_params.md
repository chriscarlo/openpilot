...
**The envelope (selectively bidirectional).** While NO hazard/urgency gate is active, the per-frame DOWNWARD (comfort-braking-onset) move of `output_a_target` is bounded to `ComfortJerkLimitMps3 · dt` (0.8 m/s³ · 0.05 s = 0.04 m/s²/frame) — the road's headline defect is a sudden unnecessary BRAKE (−0.31 → −1.00), and that felt brake blip is what the envelope kills. The same base bound applies to the first frame of an UPWARD micro-move only when planner ownership proves that the discrete `lead_keepup` or `lead_brake_release` floor raised and still owns the final pre-envelope target. Only the small span from the configured near-target regen floor to the keep-up ceiling qualifies; larger recovery demands pass immediately. Consecutive frames progressively open authority from the same comfort-jerk value so a persistent keep-up request reaches full authority without changing macro following. This smooths the captured +0.05 → +0.22 m/s² lead-follow throttle tap without rate-limiting ordinary MPC, continuous gap-reclaim, large recovery, or low-speed launch acceleration. Measured (repro oracle): the un-enveloped peak downward comfort-brake jerk of ~4.3 m/s³ drops to ~0.8 m/s³, and the first floor-owned upward pulse drops below 1.0 m/s³ while the rollback twin preserves it above 1.5 m/s³.
