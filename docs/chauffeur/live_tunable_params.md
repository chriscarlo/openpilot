...
| `LeadBrakeReleaseJerkMps3` | 2.0 | 0.0–10.0 | Hyundai-only positive jerk cap while a continuous same-track lead-follow command releases an existing brake above `LeadBrakeReleaseMinSpeedMps`. The positive step uses consecutive `modelV2` monotonic timestamps, so 28–71 ms publication jitter still produces the configured jerk; a first, missing, nonmonotonic, nonfinite, or >4-cycle-stale clock falls back to nominal `DT_MDL`. New/downward braking and fixed-frame state machines remain nominal and are never delayed; source/track/low-speed changes disarm immediately. `0` restores the pre-cap release path exactly |
...
The governor (radard `ModelLeadTrack._update_closing_governor`) keeps a windowed `(t, raw dRel, raw vRel, raw aLead)` evidence deque and latches when EITHER the endpoint-mean position slope of the raw dRel window closes faster than the currently PUBLISHED closing speed by `ClosingGovernorMarginMps` (position-excess path), OR the windowed mean raw lead accel is below −`ClosingGovernorAccelOnsetMps2` (sustained-decel path, the earliest reliable road signal) — BOTH paths requiring the windowed raw vRel to agree the lead is closing by more than `ClosingGovernorMinClosingMps`. While latched (held `ClosingGovernorHoldS` past the last qualifying frame), closing urgency is forced to 1, `aLeadK` runs at `ClosingGovernorALeadTauS`, and the published `vLead` is one-directionally clamped toward corroborated closure. Full `ClosingGovernorPosTrustExcessMps` authority requires independent evidence from sustained raw lead braking at the same `ClosingGovernorAccelOnsetMps2` threshold (window mean or two consecutive samples), or a short TTC computed from current raw vRel; a TTC computed from the same position slope cannot corroborate itself. The smaller `OpeningGovernorALeadVetoMps2` still marks current braking immediately and revokes every less-urgent recovery/opening path, but it cannot spend the full position allowance. Otherwise position authority grows continuously only by windowed raw-vRel closure above `ClosingGovernorMarginMps`; one unconfirmed current raw-aLead sample may add only magnitude-based bridge authority capped by `ClosingGovernorUnconfirmedALeadTrustMps`. **SAFETY: nothing is fabricated — the fast taus still filter the model's own measurements and the clamp can only make the published lead SLOWER / more urgent.** Noise immunity is structural: single heavy-tail dRel outliers (road ±1.5–3 m frames) cannot dominate k-endpoint means over the window, and a raw-position collapse with calm vRel/aLead remains capped at velocity evidence. Rollback sentinels: `ClosingGovernorMarginMps >= 99` disables the governor entirely; `ClosingGovernorAccelOnsetMps2 >= 99` disables both accel-derived arm and full-position-trust paths while position/velocity/short-TTC paths remain; `ClosingGovernorUnconfirmedALeadTrustMps = 0` disables only the one-frame bridge. Harnesses: the road-braking repro retains its 1.00 s minimum-THW floor, while the route-22a position-collapse repro removes the pre-fix −1.00 m/s² false brake and 1.68 m true-gap expansion.
...
| `ClosingGovernorAccelOnsetMps2` | 0.35 | 0.05–100.0 | Sustained-decel arm and full-position-trust threshold: window mean below −this arms the governor, and either the mean or two consecutive samples below it may grant full position authority (road separation: −0.55 braking vs −0.10 steady). Sentinel `>= 99` disables both accel-derived paths, while position/velocity/short-TTC paths remain |
...
The opening governor watches the SAME raw evidence window CD9 trusts and, when the k-endpoint mean slope of raw dRel proves a sustained opening with no threat veto standing, floors the published `vRel` at `min(pos_opening − TrustDeficitMps, 0.0)` — one-directional (`max()` at publish: only ever less urgent), capped at parity, publish-time only (internal EMA/association state untouched). Exact mirror of CD9's `min()` clamp with the same position-primacy rationale. `OpeningGovernorHoldS` turns that proof into a bounded Schmitt hold: its absolute deadline is refreshed only by another dense RAW-position opening proof, never by filtered or published state. Bridging also requires a separate 1.0–1.5 s raw-position trend to remain non-closing; that longer trend can only end a hold early and can never arm or refresh one. A previously earned hold may bridge a weak, position-only CD9 latch, but cannot arm or refresh while CD9 is active; corroborated CD9 braking/closure always wins. Same-frame hard exits independently revoke the hold for raw lead braking, fast/short-TTC closure, oncoming velocity, lateral migration/cut-in, near-threat range, missed/reassociated/reordered tracks, or a published TTC under 6 s. The ordinary window vetoes remain windowed raw vRel closing beyond `RawClosingVetoMps` and windowed raw aLead below −`ALeadVetoMps2`.
...
### Calm-Recovery Brake-Release Bridge

After a genuinely corroborated closing episode, RadarD can prove from a dense
long-window position history plus consecutive calm raw measurements that its
public filtered `vRel` remains unnecessarily pessimistic. RadarD continues to
publish the conservative kinematics, but also publishes the numeric position
closure and a position-only recovery vRel floor. The Hyundai planner may use
that evidence only to nominate its existing coast-bias brake-release floor,
outside the exact configured target gap, and only while both the current
control lead and untouched RadarState copy pass acceleration, TTC, range,
lateral, probability, source, identity, and FCW gates. The ordinary release
floor, slowdown ceilings, planner clip, and all CD5/CD6/CD7 limiter state run
unchanged first. A final isolated correction can then raise the rollback output
by at most `0.10 m/s²`, subject to the existing positive release-jerk tune and
hard-brake/FCW/force-decel/stop vetoes. It never changes classification, MPC
obstacles, source ownership, steady parity, FCW, or any limiter anchor. Renewed
threat publishes the ordinary rollback output in the same planner frame. The
tune is default-off for a bounded on-road canary after replay and safety-twin
validation.

| Param Key | Default | Range | Description |
|---|---|---|---|
| `ClosingRecoveryBridgeMaxPositionClosingMps` | 0.0 | 0.0–1.25 | Maximum independent long-window position closure allowed for the positive brake-release floor. `0` disables the bridge exactly; `1.25` is the reviewed candidate and hard safety maximum |
...
An isolated far/high-TTC raw-vRel spike removes the private correction on that
exact frame and clears the proof epoch; correction must earn a fresh dense
position window before it can rearm. Raw vRel alone never triggers the planner's
comfort-bypass attestation. Current braking, short TTC, near/oncoming geometry,
or a same-frame position-confirmed closure does, so genuine restored braking is
not weakened by stateful comfort carryover.
...
**(1) Symmetric windowed delta clamp (road 200-6/22d).** A vLeadK rollover on a FAR non-hazard lead (dRel > 75 m, TTC > 100 s) briefly makes the fresh lead0 obstacle undercut cruise; the MPC's cruise-owned accel cap COLLAPSES from a high value to ~0 in one frame and slams `aTarget` from ~+0.58 to ~−0.56 — one to a few frames BEFORE the source label flips to lead0. A separate first-visible opening-lead edge makes the cap appear from `None` and can create the same actuator-delay inversion without a source flip. The window arms on (a) a real cruise↔lead source flip, (b) a large single-frame cruise-owned-cap drop, or (c) first cap appearance when every valid control lead is non-closing. While armed for `HandoffLimitWindowS`, `|output_a_target − prev_a|` is bounded to `HandoffLimitMaxDeltaMps2` per frame. **Asymmetric bypass:** the UPWARD leg always applies; the DOWNWARD leg includes both sign flips and abrupt positive-throttle reductions, but evaluates every valid lead and bypasses immediately for no-lead dropout, FCW, short TTC, kinematic/fast closing, lead decel, or a hard requested decel. Measured (noise-off oracle): the road's ~−1.13 m/s² flip drops to −0.30; the first-visible opening-lead harness changes +1.50→−0.70 into +1.50→+1.20 without delaying its matched braking twin.
...
| Param Key | Default | Range | Description |
|---|---|---|---|
| `HandoffLimitWindowS` | 0.40 | 0.0–1.0 | Duration (s) the symmetric per-frame delta clamp is armed after a cruise↔lead source flip, cruise-owned-cap collapse, or all-opening first-cap appearance. 0 disables the windowed limiter (pre-CD6 hard handoff) |
| `HandoffLimitMaxDeltaMps2` | 0.30 | 0.0–2.0 | Max `|output_a_target − prev_a|` (m/s²) per frame while the window is active, including benign positive-throttle reductions. Downward moves are urgency-bypassed; also governs the EDGE1 release rate. Only meaningful when `HandoffLimitWindowS` > 0 |
...
**The envelope (selectively bidirectional).** While NO hazard/urgency gate is active, the per-frame DOWNWARD (comfort-braking-onset) move of `output_a_target` is bounded to `ComfortJerkLimitMps3 · dt` (0.8 m/s³ · 0.05 s = 0.04 m/s²/frame) — the road's headline defect is a sudden unnecessary BRAKE (−0.31 → −1.00), and that felt brake blip is what the envelope kills. The same constant bound applies to every frame of an UPWARD micro-move only when planner ownership proves that the discrete `lead_keepup` or `lead_brake_release` floor raised and still owns the final pre-envelope target. Only the small span from the configured near-target regen floor to the keep-up ceiling qualifies; larger recovery demands pass immediately. A persistent floor request therefore advances by exactly the configured comfort-jerk step each frame until satisfied. This smooths the captured +0.05 → +0.22 m/s² lead-follow throttle tap without rate-limiting ordinary MPC, continuous gap-reclaim, large recovery, or low-speed launch acceleration. Measured (repro oracle): the un-enveloped peak downward comfort-brake jerk of ~4.3 m/s³ drops to ~0.8 m/s³, and the floor-owned upward pulse remains below 1.0 m/s³ on every qualifying frame while the rollback twin preserves the original tap above 1.5 m/s³.
...
| `LeadAccelCorrAmplifyModelDecelMinMps2` | 0.10 | 0.0–1.0 | Minimum model-reported braking magnitude before the vLead-trend amplifier may deepen aLeadK. Rejects near-zero sign noise that caused transient brake taps; 0 restores any-negative-report behavior |
...
The same low-passed vLead-trend finite-difference (`a_meas`) that the bound above uses to *cap* an uncorroborated decel is also used to *amplify* a corroborated one. The model's `leadsV3.a` chronically underreports real lead braking (road unit 200-13: published aLeadK peaked -0.48 m/s^2 while the position-derived truth was -1.3..-2.3, and radard's 0.6 s accel EMA halves it again); the vLead trend measures the true decel from vLead alone (in that event it tracked -1.7..-1.9). Amplification requires the model report to be more negative than `-AmplifyModelDecelMinMps2` **and** the trend to be more negative than that report by `AmplifyDeadbandMps2`; aLeadK is then pulled an `AmplifyGain` fraction toward the trend (never past it, never by more than `AmplifyCapMps2` in one frame). The independent magnitude floor rejects near-zero sign noise that previously let the same derivative fabricate -1.6..-1.9 m/s^2 brake evidence from -0.017/-0.04 reports.

RadarD also publishes an independent dense-position proof for the narrow case where the derivative alone claims hard deceleration: at least 24 samples / 1.6 s of same-track position history, slope -1.25..+0.75 m/s, current range-step closure no greater than 1.25 m/s, high probability, centered/low-lateral geometry, long TTC, and mild public `aLeadK`. When that proof is fresh and the lead remains outside the exact configured target, the Hyundai MPC skips only the **extra** CD3 amplification; public RadarState, source selection, obstacle geometry, FCW, the ordinary correlation bound, and the model's own aLeadK remain unchanged. Same-frame raw aLead below -0.40 clears the proof, preserving the known -0.48..-0.54 CD3 truth-deficit lane. On the 2026-07-15 17:43:09.997 false onset this changes `aTarget` from -1.000 to -0.0005 m/s²; matched genuine closures at 17:39:25, 17:40:40, and 17:45:02 fail the position/closing gates and remain bit-identical to proof-disabled behavior.

RadarD separately marks a calm-recovery epoch on `RadarState.LeadData.closingGovernorRecovery`; the MPC clears its derivative correlation while the bit is true and requires a fresh ~0.6 s unshaped history afterward, so the 2026-07-13 route-406 governor-shaped vLead history cannot corroborate its weak -0.122 report into a false -2.122 m/s^2 brake. The genuine CD3 braking oracle remains safe when recovery ends immediately before its brake: amplification resumes after the bounded re-settle, planner onset remains 0.45 s, and min THW/TTC remain 0.95 s/4.52 s. Unlike the downward bound, amplification still runs in a dangerous/closing state once both corroboration gates pass. On the deficit repro (true decel -1.8, model reports 0.3x) it holds min THW ~0.99 s / min TTC ~4.9 s versus the bound-only 0.49 s / 2.4 s. `AmplifyGain=0` disables amplification; `AmplifyModelDecelMinMps2=0` restores the prior any-negative-report eligibility.
