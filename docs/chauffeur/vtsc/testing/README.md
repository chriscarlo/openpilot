...
Purpose
- Replays recorded rlogs through the current VisionTurnController (VTSC), capturing a method‑level trace of internal state changes and the final VTSC snapshot for each model frame.
...
- Optional flags:
  - `--cruise <mps>`: Constant cruise setpoint; defaults to `v_ego` if omitted.
  - `--max-frames N`: Limit frames for iteration.
