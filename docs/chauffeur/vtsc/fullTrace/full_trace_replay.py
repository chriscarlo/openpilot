#!/usr/bin/env python3
# moved from docs/chauffeur/vtsc/testing/full_trace_replay.py
...
ATTRS_CURV = ["_filtered_curvature", "_current_lat_acc", "_max_pred_lat_acc"]
ATTRS_GATING = ["_fov_occluded", "_dbg_psi_vis", "_dbg_psi_thresh", "_psi_fov_rad", "_psi_margin_rad", "_freeway_failopen_active"]
ATTRS_CAPS = ["_dbg_active_cap", "_dbg_cap_visible_vmin", "_dbg_cap_occl_vmin", "_dbg_cap_map_vmin", "_dbg_vtsc_cmd", "_dbg_tail_frac", "_dbg_s_tail", "_pre_cap_target_speed", "_dbg_pre_cap_target"]
ATTRS_TUNABLES = ["_vis_horizon_s", "_vis_margin_m", "_lat_jerk_cap", "_anticipation_target_reduction", "_anticipation_max_reduction_mps", "_onset_no_raise_active", "_occlusion_onset_timer_s", "_v_cap_active_at_onset_mps"]
...
def _extract_occl_state(vtsc: VisionTurnController) -> Dict[str, Any]:
  ...
  return {f: _safe_get(occl, f) for f in fields}
