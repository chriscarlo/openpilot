...
schemas from a provided repo root (auto-detected from script location by default).
"""
  ...
  *,
  ...
  def _row_for_target(target_t: float, ev: EventBundle) -> Dict[str, Any]:
    ...
    row: Dict[str, Any] = {
      ...
      # GPS/map observability for MTSC RCA
    ...
    # VTSC debug fields (from logMessage/VTSCDBG snapshots).
    vdbg = _get_latest("vtscdbg")
    row["activeCap"] = vdbg.get("activeCap")
    row["capVisibleVmin"] = vdbg.get("capVisibleVmin")
    row["capOcclVmin"] = vdbg.get("capOcclVmin")
    row["capMapVmin"] = vdbg.get("capMapVmin")
    row["mapTailActive"] = vdbg.get("mapTailActive")
    row["mapTailCoverage"] = vdbg.get("mapTailCoverage")
    row["mapTailStartM"] = vdbg.get("mapTailStartM")
    row["visionStatus"] = vdbg.get("visionStatus")
    row["dbgConf"] = vdbg.get("dbgConf")
    row["psiVis"] = vdbg.get("psiVis")
    row["psiThresh"] = vdbg.get("psiThresh")
    row["dbgVBase"] = vdbg.get("dbgVBase")
    row["dbgVVis"] = vdbg.get("dbgVVis")
    row["dbgVOcc"] = vdbg.get("dbgVOcc")
    row["dbgRaw"] = vdbg.get("dbgRaw")
  ...
  for evt in _iter_capnp_events(rlog_path, capnp_log):
    ...
    if which == "carState":
      ...
      try:
        log_str = evt.logMessage
        outer = json.loads(log_str)
        msg = outer.get("msg", "")
        if isinstance(msg, str) and msg.startswith("VTSCDBG "):
          snap = json.loads(msg.split("VTSCDBG ", 1)[1])
          if isinstance(snap, dict):
            latest["vtscdbg"] = {
              "activeCap": str(snap.get("active_cap") or ""),
              "capVisibleVmin": _safe_float(snap.get("cap_visible_vmin")),
              "capOcclVmin": _safe_float(snap.get("cap_occl_vmin")),
              "capMapVmin": _safe_float(snap.get("cap_map_vmin")),
              "mapTailActive": bool(snap.get("map_tail_active", False)),
              "mapTailCoverage": _safe_float(snap.get("map_tail_coverage")),
              "mapTailStartM": _safe_float(snap.get("map_tail_start_m")),
              "visionStatus": str(snap.get("vision_status") or ""),
              "dbgConf": _safe_float(snap.get("conf")),
              "psiVis": _safe_float(snap.get("psi_vis")),
              "psiThresh": _safe_float(snap.get("psi_thresh")),
              "dbgVBase": _safe_float(snap.get("v_base")),
              "dbgVVis": _safe_float(snap.get("v_vis")),
              "dbgVOcc": _safe_float(snap.get("v_occ")),
              "dbgRaw": _safe_float(snap.get("raw")),
...
def _str_at_dt(df: pd.DataFrame, dt: float, col: str) -> str | None:
  """Like _value_at_dt but returns the raw value as a string (for non-numeric columns)."""
  if df.empty or col not in df.columns or "dt" not in df.columns:
    return None
  want = float(f"{dt:.2f}")
  sub = df[df["dt"].sub(want).abs() < 1e-9]
...
def _compute_summary_row(ev: EventBundle, df: pd.DataFrame) -> Dict[str, Any]:
  ...
  # Map data age max in pre-window
  ...
  # Map road name changes in pre-window
  ...
  # Map speed limit valid changes in pre-window
  ...
  # Map road geometry valid any in pre-window
  ...
  # VTSC debug attribution at -0.5s (from VTSCDBG logMessage snapshots).
  row["activeCap@-0.5"] = _str_at_dt(df, -0.5, "activeCap")
  row["capVis@-0.5"] = _value_at_dt(df, -0.5, "capVisibleVmin")
