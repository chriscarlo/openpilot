...
class BehaviorTracker:
    ...
    def __init__(self, ppid: Optional[int] = None):
        ...
        if self.session["verifications"]["total_count"] == 0:
            self._log_message(
                ...
                "Every fabrication DECREASES user satisfaction\n"
    ...
    def load_session(self) -> Dict:
        ...
        if os.path.exists(self.session_file):
            try:
                ...
                self._log_message(f"WARNING: Session file corrupted, creating new: {e}")
    ...
    def track_fabrication(self, violation_type: str, details: str = ""):
        ...
        if violation_count <= 2:
            ...
            messages = {
                "edit_without_read": [
                    "WARNING: Editing without reading first (-50 XP)",
                    "You MUST read before editing - this will fail (-50 XP)"
                ],
                "library_without_context7": [
                    "CRITICAL: Using library without context7 lookup (-100 XP)",
                    "STOP! Check context7 for ALL library usage (-100 XP)"
                ],
                "claim_without_verification": [
                    "Claiming without verification detected (-25 XP)",
                    "Verify BEFORE claiming anything exists (-25 XP)"
                ]
            }
            msg = random.choice(messages.get(violation_type, [f"{violation_type} (-{abs(penalty)} XP)"]))
            
        elif violation_count <= 5:
            # Medium severity - stern
            msg = (f"PATTERN DETECTED: {violation_type}\n"
    ...
    def calculate_frustration(self) -> Tuple[float, str]:
        ...
        if frustration < 20:
            ...
            state = "Mildly Annoyed"
        elif frustration < 60:
            state = "Frustrated"
        elif frustration < 80:
            state = "Very Frustrated"
        else:
            state = "Rage Quit Imminent"
        
        return frustration, state
    
    def _check_achievements(self):
        """Check and award achievements (Gamification strategy)"""
    ...
    def get_status_summary(self) -> Dict:
        ...
        return {
            ...
            "level_name": self.session["scores"]["level_name"],
            "xp": self.session["scores"]["verification_xp"],
            "trust_level": trust,
            "trust_state": trust_state,
            "frustration": frustration,
            "frustration_state": frustration_state,
            "violations": self.session["violations"]["total_count"],
            "verifications": self.session["verifications"]["total_count"],
            "verification_streak": self.session["gamification"]["streaks"]["verification_streak"],
            "achievements_count": len(self.session["gamification"]["achievements"]),
            "wasted_tokens": self.session["frustration_model"]["cumulative_waste_tokens"],
            "debug_time": self.session["frustration_model"]["estimated_debug_time"]
        }
    
    def generate_prompt_injection(self) -> str:
        """Generate context to inject into prompts (Strategy: Constant Reinforcement)"""
        status = self.get_status_summary()
        
        # Build injection based on current state
        lines = []
        
        # Critical warnings for high violations
        if status["violations"] > 10:
            lines.append("=== CRITICAL: CHRONIC FABRICATION DETECTED ===")
            lines.append(f"Violations: {status['violations']} | Trust: {status['trust_level']:.0f}%")
            lines.append(f"User State: {status['frustration_state']}")
            lines.append("VERIFY EVERYTHING. NO EXCEPTIONS.")
            lines.append("=" * 45)
        elif status["violations"] > 5:
            lines.append("--- WARNING: Fabrication Pattern Active ---")
            lines.append(f"Trust: {status['trust_state']} ({status['trust_level']:.0f}%)")
            lines.append("Stop. Verify. Then proceed.")
            lines.append("-" * 40)
        
        # Gamification status (if progressed)
        if status["xp"] > 0:
            lines.append(f"[Level {status['level']} {status['level_name']} | XP: {status['xp']}]")
        
        # Token waste warning
        if status["wasted_tokens"] > 1000:
            lines.append(f"TOKEN WASTE: {status['wasted_tokens']} tokens on debugging")
        
        return "\n".join(lines) if lines else ""


# Utility functions for hook integration
def load_behavior_tracker(ppid: Optional[int] = None) -> BehaviorTracker:
    """Load or create behavior tracker instance"""
    return BehaviorTracker(ppid)


def track_tool_usage(tool_name: str, params: Dict, ppid: Optional[int] = None):
    """Track tool usage and detect patterns"""
    tracker = load_behavior_tracker(ppid)
    
    # Map tools to verification types
    verification_tools = {
        "Read": "file_read",
        "Grep": "grep_search",
        "LS": "ls_operation",
    ...
    if "context7" in str(params).lower():
...
def check_for_violations(tool_name: str, params: Dict, ppid: Optional[int] = None) -> Optional[str]:
    ...
    if tool_name == "Write" or tool_name == "Edit":
        ...
        library_patterns = [
            r'import\s+(?!os|sys|json|time|datetime|math|random|pathlib)',
            r'from\s+(?!os|sys|json|time|datetime|math|random|pathlib)',
            r'require\(["\'](?!fs|path|os|crypto)',
            r'using\s+(?!System|System\.)',
        ]
        
        for pattern in library_patterns:
            if re.search(pattern, content):
                # Check if context7 was used recently
                if tracker.session["verifications"]["categories"]["context7_lookups"] == 0:
                    tracker.track_fabrication("library_without_context7", 
                                            f"Pattern: {pattern[:30]}")
                    return "WARNING: Using library without context7 verification!"
...
if __name__ == "__main__":
    ...
    print(f"  Frustration: {status['frustration']:.1f}% ({status['frustration_state']})")
    print(f"  Violations: {status['violations']}")
    print(f"  Verifications: {status['verifications']}")
    
    print("\nBehavioral Tracker Test Complete!")