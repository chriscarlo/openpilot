  ...
  try:
    ...
    for frame in range(frames):
      ...
      if bool(args.closed_loop):
        a_cmd = float(np.clip(a_solution, float(args.min_accel), float(args.max_accel)))
        v_ego = max(0.0, v_ego + a_cmd * dt_s)
        a_ego = a_cmd
  finally:
    long_mpc_module.time.monotonic = original_monotonic

  summary = summarize(samples, hz=float(args.hz))
...
def print_summary(summary: dict[str, Any], samples: list[Sample], args: argparse.Namespace) -> None:
  ...
  print(
    ...
    f"source_counts={summary['source_counts']} reset_counts={summary['reset_counts']}"
  )
  print(
