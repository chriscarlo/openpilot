...
The controller deliberately rejects legacy `MapPreCurveSpeeds`. Schema-v1 tile speeds are derived from raw triplets, while current mapd publishes route-aware curvature; pairing them would reintroduce the same false slowdowns the calibration audit removes. `whole-curve-v3` is the estimator-aligned replacement and keeps the legacy stream disabled.
