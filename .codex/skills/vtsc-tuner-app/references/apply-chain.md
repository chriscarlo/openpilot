...
5. If requested, transfers the canonical tile artifact only to an empty remote partial set, checks every manifest entry and digest on-device, installs one immutable generation at `/data/media/0/osm/tile-generations/<tile-set-id>/`, and atomically switches the active `offline` pointer while retaining the prior generation. If the starting tree is a direct legacy `offline/` directory, the native helper first durably writes a target-bound transaction, then creates the immutable migration, preserving a valid adjacent canonical tile identity when present (otherwise generating `legacy-*`), and returns target/provenance proof with the exact prior ID. The app validates and durably journals it before any later step. A direct adjacent identity equal to the requested target returns an explicit no-switch only after the helper proves every requested file and digest matches; disagreement fails before a transaction, generation, switch link, or exchange. Fresh nil-prior recovery accepts either the exact durable helper transaction or a fully validated post-rollback topology containing the immutable migrated prior, `offline.previous` at the exact deployed target, both content identities, and retained-tree provenance; a matching active manifest alone is insufficient. If activation never began, an unchanged direct tree with no transaction, prior pointer, embedded manifest, or exchange artifacts returns an exact no-switch with nil prior identity.
