...
- Added a separate native **Resume Pending Outdoor Postflight** action for a schema-1 journal that is incomplete only after its deployment reboot. It keeps the journal's immutable deployed target, permits a newer exact local/origin/tici tooling head only when Git proves every intervening path is under the native tuner or its skill, and uses a two-phase read-only observer: first it captures a fresh same-tune profile plus newly updated, valid `liveMapDataSP.roadGeometryValid=true` evidence while ignition is on, rejecting messages more than 1.5 seconds old or from the future; it then explicitly tells the user when to turn ignition off and retains that proof only in memory while waiting for offroad. Completion fully parses the offroad profile again and revalidates the unchanged Git/release/tune/mapd/tile/profile identity. It never redeploys or reboots and atomically marks that same journal complete only after every check passes. Completion records the validated tooling head and host-only diff paths without changing `targetHead`; a stable per-journal lock closes multi-app races, and active Cancel or any indoor/mismatch failure preserves the pending journal byte-for-byte. Empty indoor `{}` profiles report pending GPS/profile instead of a misleading estimator/hash mismatch.
