...
- Added a separate native **Resume Pending Outdoor Postflight** action for a schema-1 journal that is incomplete only after its deployment reboot. It keeps the journal's immutable deployed target, permits a newer exact local/origin/tici tooling head only when Git proves every intervening path is under the native tuner or its skill, performs two read-only device/source/Git/release/tune/tile/profile/branch/cleanliness checks with the existing real-GPS validator, never redeploys or reboots, and atomically marks that same journal complete only after every check passes. Completion records the validated tooling head and host-only diff paths without changing `targetHead`; a stable per-journal lock closes multi-app races, and active Cancel or any indoor/mismatch failure preserves the pending journal byte-for-byte.
