...
Abort and Recover take global then per-journal ownership, exclude every peer VTSC Tuner process and every other unresolved journal before local lifecycle mutation and again immediately before rollback ownership/device mutation. The current tici HEAD must be exactly the recorded prior head (idempotent replay), the deployed target, or one completion-compatible descendant proven by the same host Git rule. Tile recovery stages its content-addressed helper only behind that exact binding, rechecks Git after taking the on-device tile lock, and a target/tile precondition failure stops all later Git, Params, and mapd restoration.
