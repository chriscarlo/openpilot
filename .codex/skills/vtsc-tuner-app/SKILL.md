...
Abort and Recover take global then per-journal ownership, exclude every peer VTSC Tuner process and every other unresolved journal before local lifecycle mutation and again immediately before rollback ownership/device mutation. At each checkpoint, peer exclusion happens before safely pruning any aged targetless legacy preflight; the selected journal is excluded by both URL and immutable deployment identity. The current tici HEAD must be exactly the recorded prior head (idempotent replay), the deployed target, or one completion-compatible descendant proven by the same host Git rule. Tile recovery stages its content-addressed helper only behind that exact binding, rechecks branch, HEAD, and an empty worktree after taking the on-device tile lock, and a target/tile precondition failure stops all later Git, Params, and mapd restoration. When first activation migrates a direct legacy `offline/` tree, the helper preserves a valid adjacent canonical ID or creates a synthetic `legacy-*` ID, embeds target-bound migration provenance, and the app durably journals that exact prior identity plus the verified `switched` / `notSwitched` outcome before any continuation. The helper durably writes target-bound transaction authority before creating immutable generations or publishing a switch link, so every crash boundary has an exact recovery authority. Before clearing a pre-exchange transaction it validates and durably removes every helper-owned generation, build, switch, temporary-link, and retained artifact; a lost successful no-switch reply therefore replays from clean topology. A fresh nil-prior recovery accepts either that transaction proof or a complete post-rollback topology with exact active-prior provenance, `offline.previous` target identity, content digests, and retained-tree evidence; an active manifest alone never suffices. With no transaction or artifacts, a real direct tree may prove only the exact valid adjacent ID already recorded in the journal, which the helper reports as both active and previous; missing or mismatched identity fails closed. If a direct tree's adjacent canonical identity already equals the requested target, activation proves the complete content first and returns an explicit no-switch without creating or exchanging pointers; mismatched same-ID content fails before mutation but remains a provable unchanged no-switch for restoring earlier Git/Params/mapd legs.
