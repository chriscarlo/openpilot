...
Resume always requires the tici at the journal's immutable deployed target HEAD. A later local/origin tooling HEAD is accepted only when its diff from that deployed target is proven host-only and is recorded separately as completion metadata. New `awaitingOutdoorPostflight` journals also re-prove that the current boot ID differs from their durable deployment-preboot ID during both controller-ready and final offroad reads. The only missing-boot-ID compatibility exception is a legacy schema-1 `awaitingPostflight` journal such as B174; it remains resumable using current supervised controller evidence without fabricating or persisting historical boot data.
