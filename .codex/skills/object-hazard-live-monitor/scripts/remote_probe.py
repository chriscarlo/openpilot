...
if "__file__" in globals():
  ...
  REPO_ROOT = _candidate.parents[4] if len(_candidate.parents) >= 5 else Path.cwd()
else:
  REPO_ROOT = Path.cwd()
...
def main() -> None:
  ...
  while time.monotonic() < deadline:
    ...
    if "logMessage" in sm.data and sm.updated.get("logMessage", False):
      log_msg = sm["logMessage"]
      log_line = safe_text(getattr(log_msg, "logMessage", log_msg)).strip()
    ...
    if device is not None:
      ...
      thermal = safe_int(getattr(device, "thermalStatus", 0))
