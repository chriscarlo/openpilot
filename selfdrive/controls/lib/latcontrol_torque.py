...
class LatControlTorque(LatControl):
  def __init__(self, CP, CI):
    ...
    if self.use_nnff or self.use_nnff_lite:
      ...
      self.friction_look_ahead_v = [0.5, 3.0] # how many seconds in the future to look ahead in [0, ~2.1] in 0.1 increments
      self.friction_look_ahead_bp = [1.0, 30.0] # corresponding speeds in m/s in [0, ~40] in 1.0 increments

      # Scaling the lateral acceleration "friction response" could be helpful for some.
      # Increase for a stronger response, decrease for a weaker response.
      self.lat_jerk_friction_factor = 3.2
